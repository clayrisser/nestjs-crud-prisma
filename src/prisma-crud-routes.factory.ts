...
} from '@nestjsx/crud/lib/interceptors';
import {
  BaseRoute,
  CrudOptions,
  CrudRequest,
...
export class PrismaCrudRoutesFactory {
  ...
  private canCreateRoute(name: string) {
    // @ts-ignore
    const { only } = this.options.routes;
    // @ts-ignore
    const { exclude } = this.options.routes;
  ...
  private setInterceptors(name: BaseRouteName) {
    // @ts-ignore
    const { interceptors } = this.options.routes[name];
    R.setInterceptors(
      [
        CrudRequestInterceptor,
        CrudResponseInterceptor,
  ...
  private setDecorators(name: BaseRouteName) {
    // @ts-ignore
    const { decorators } = this.options.routes[name];
    R.setDecorators(
      // @ts-ignore
